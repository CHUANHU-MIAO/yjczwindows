...
impl App {
    pub fn show_tools(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Grid::new("tools_grid")
            ...
            .show(ui, |ui| {
                ...
                // Rufus 启动盘制作
                if ui
                    .add(egui::Button::new("📀 Rufus\n启动盘制作").min_size(egui::vec2(120.0, 60.0)))
                    .clicked()
                {
                    self.launch_rufus();
                }
    ...
    fn launch_rufus(&mut self) {
        let bin_dir = crate::utils::path::get_bin_dir();
        let rufus_path = bin_dir.join("rufus.exe");

        if rufus_path.exists() {
            match Command::new(&rufus_path).spawn() {
                Ok(_) => {
                    self.tool_message = "已启动: Rufus 启动盘制作工具".to_string();
                }
                Err(e) => {
                    self.tool_message = format!("启动失败: Rufus - {}", e);
                }
            }
        } else {
            // 尝试在 bin/tools 目录下查找
            let tools_rufus = crate::utils::path::get_tools_dir().join("rufus.exe");
            if tools_rufus.exists() {
                match Command::new(&tools_rufus).spawn() {
                    Ok(_) => {
                        self.tool_message = "已启动: Rufus 启动盘制作工具".to_string();
                    }
                    Err(e) => {
                ...
                self.tool_message = "Rufus 未安装，请将 rufus.exe 放入程序 bin 目录".to_string();
            }
        }
    }
